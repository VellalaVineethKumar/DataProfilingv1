...
def _save_persisted_data():
    ...
    try:
        ...
        persist_data = {
            ...
            'unified_validation_rules': state.unified_validation_rules,
            'unified_rules_generated': state.unified_rules_generated,
...
def _restore_persisted_data():
    ...
    try:
        ...
        state.unified_validation_rules = persist_data.get('unified_validation_rules')
        state.unified_rules_generated = persist_data.get('unified_rules_generated', False)
        ...
        state.fixes_applied = persist_data.get('fixes_applied', [])
        state.operation_count = persist_data.get('operation_count', 0)
        state.last_operation = persist_data.get('last_operation')
        ...
        if not toast['shown']:
            try:
                if toast['type'] == 'success':
                    st.toast(toast['message'], icon=None)
...
def update_dataframe(new_df: pd.DataFrame, operation: str):
    ...
    try:
        ...
        log_action(
            ...
            username=getattr(state, "username", None) or "system",
...
def clear_data_only():
    """Clear only data-related state while preserving authentication"""
    try:
        state = st.session_state.app_state
        
        # Clear data-related fields only
        state.df = None
        ...
        state.detected_dq_rules = {}
        state.column_profiles = {}
        state.quality_report = None
        state.exact_duplicates = []
        state.fuzzy_duplicates = []
        state.combined_duplicates = []
        state.ai_validation_rules = None
        ...
        # Sync flat state keys
        _sync_flat_state_from_appstate()
        
        show_toast("Data cleared successfully", "success")
    except Exception as e:
        show_toast(f"Error clearing data: {str(e)}", "error")
