...
def render_export():
    ...
    with col3:
        ...
        if len(df) > 100000:
            ...
            sample_pct = st.slider("Sample %:", 1, 100, 100, key="exp_sample_pct")
        else:
            sample_pct = 100
    ...
    with st.expander("Advanced Options"):
        ...
        with adv_col1:
            include_index = st.checkbox("Include Index", value=False, key="exp_include_idx")
            encoding = st.selectbox("Encoding:", ["utf-8", "latin-1", "utf-16"], key="exp_encoding")
        ...
        with adv_col3:
            if export_format == "CSV":
                delimiter = st.selectbox("Delimiter:", [",", ";", "\t", "|"], key="exp_delimiter")
                quotechar = st.selectbox("Quote char:", ['"', "'"], key="exp_quotechar")
    ...
    csv_delimiter = st.session_state.get("exp_delimiter", ",")
    csv_quotechar = st.session_state.get("exp_quotechar", '"')
    
    if st.button("Generate Export File", type="primary", use_container_width=True, key="exp_generate"):
    ...
    if len(df) > 500000:
        ...
        rows_per_file = st.number_input("Rows per file:", 
                                       ...
                                       value=100000,
                                       step=10000,
                                       key="exp_rows_per_file")
        
        if st.button("Generate Batch Export", use_container_width=True, key="exp_batch_generate"):
            _generate_batch_export(df, export_format, rows_per_file, export_cols or df.columns.tolist())
    
    # PDF / HTML Profiling Report
    st.divider()
    st.markdown("### Profiling Report")
    st.caption("Download a formatted profiling report with executive summary, column profiles, and recommendations.")

    report_cols = st.columns(2)
    with report_cols[0]:
        if st.button("Generate HTML Report", use_container_width=True, key="exp_html_report"):
            from features.export.pdf_report import generate_profiling_report_html
            ...
            st.download_button(
                ...
                file_name=f"profiling_report_{datetime.now():%Y%m%d_%H%M%S}.html",
                mime="text/html",
                use_container_width=True,
                key="exp_dl_html_report",
            )
    with report_cols[1]:
        if st.button("Generate PDF Report", use_container_width=True, key="exp_pdf_report"):
            from features.export.pdf_report import generate_profiling_report_html, html_to_pdf_bytes
            ...
            if pdf_bytes:
                st.download_button(
                    "Download PDF Report",
                    data=pdf_bytes,
                    file_name=f"profiling_report_{datetime.now():%Y%m%d_%H%M%S}.pdf",
                    mime="application/pdf",
                    use_container_width=True,
                    key="exp_dl_pdf_report",
                )
            else:
                st.warning("PDF generation requires the xhtml2pdf package. Install with: pip install xhtml2pdf")
                st.download_button(
        ...
        if format_ == "CSV":
            if compression != "none":
                ...
                data = df.to_csv(index=include_index, encoding=encoding,
                                 compression=compression, sep=delimiter, quotechar=quotechar)
            else:
...
def _generate_batch_export(df, format_, rows_per_file, columns):
    ...
    st.download_button(
        label=f"Download Batch Export ({num_files} files)",
        data=zip_buffer.getvalue(),
        file_name=f"batch_export_{timestamp}.zip",
        mime="application/zip",
        use_container_width=True,
        key="exp_download_batch"
    )
