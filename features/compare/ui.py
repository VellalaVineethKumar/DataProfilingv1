...
def render_compare():
    ...
    # Header
    st.markdown("## Compare: Original vs Modified")
    
    # Quick stats
    row_change = len(curr_df) - len(orig_df)
    col_change = len(curr_df.columns) - len(orig_df.columns)
    modified_cells = _count_modified_cells(orig_df, curr_df)
    ...
    <div class="stat-row">
        ...
        <div class="stat-item">
            <div class="stat-label">Modified Cells</div>
            <div class="stat-value {'change-negative' if modified_cells > 0 else ''}">{modified_cells:,}</div>
        </div>
    </div>
    """, unsafe_allow_html=True)
    ...
    with col2:
        start_row = st.number_input(
            "Start row:",
            min_value=0,
            max_value=max(1, max(len(orig_df), len(curr_df))) - 1,
            value=0,
            key="start_row"
        )
...
def _count_modified_cells(orig_df: pd.DataFrame, curr_df: pd.DataFrame) -> int:
    """Count total modified cells across all common rows and columns.

    Uses vectorized comparison for performance on large datasets.
    NaN == NaN is treated as equal (not a modification).
    """
    ...
    # ne() treats NaN != NaN as False when both sides are NaN via fillna alignment,
    # so handle NaN explicitly: a cell is modified when the values differ AND
    # it's not the case that both are NaN.
