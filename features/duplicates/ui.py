    ...
    with col3:
        if st.button(
            ...
            if dup_type == "exact":
                ...
                keep_val = False if keep_strategy == "none" else keep_strategy
                transform_remove_exact_duplicates(keep=keep_val)
                st.rerun()
            else:
                st.info(
    ...
    if total_groups <= 10:
        ...
        with tabs[-1]:
            ...
            for group in duplicate_groups[5:20]:  # Show up to 20 total
                with st.expander(f"Group #{group.group_id} - {len(group.indices)} rows - {(group.similarity_score or 0):.1f}% similarity"):
                    _render_group_detail(group, dup_type)
