...
def render_dashboard():
    ...
    try:
        ...
        if qr is None:
            st.markdown('<div class="error-box">Unable to generate quality report. Please try re-uploading the file.</div>', unsafe_allow_html=True)
            return
        ...
        with insights_col1:
            ...
            if qr.missing_percentage < 5:
                ...
                health_items.append("Moderate missing data (5-20%)")
            else:
            ...
            if qr.exact_duplicate_rows == 0:
                ...
                health_items.append(f"{qr.exact_duplicate_rows:,} duplicate rows detected")
            ...
            if qr.columns_with_issues:
                st.markdown('<div class="card">', unsafe_allow_html=True)
                st.markdown(f'<div class="card-header">Columns Needing Attention ({len(qr.columns_with_issues)})</div>', unsafe_allow_html=True)
