        ...
        with st.expander(f"{col_name} ({profile.dtype})"):
            ...
            with metric_cols[3]:
                if profile.outliers.get("count", 0) > 0:
                    st.metric("Outliers", profile.outliers["count"])
                else:
            ...
            # Data Quality Tabs
            detail_tabs = st.tabs(["Overview", "Patterns", "Quality", "Special Chars", "Actions"])
            ...
            with detail_tabs[3]:
                if profile.special_chars:
                    ...
                    for char_info in profile.special_chars:
                        char_data.append({
                            'Character': char_info.get('display_name', char_info.get('key', '?')),
                    ...
                    for char_info in profile.special_chars[:5]:
                        with st.container():
                            display = char_info.get('display_name', char_info.get('key', '?'))
                            symbol = char_info.get('symbol', '')
                            st.markdown(
                                f'<div style="padding: 10px; border: 1px solid #e2e8f0; border-radius: 8px; margin: 5px 0;">'
                                f'<b>{display}</b> ({symbol}) - Found {char_info["count"]} times'
                                f'</div>',
            ...
            with action_cols[0]:
                if st.button("Drop Column", key=f"drop_{col_name}"):
            ...
            with action_cols[2]:
                if st.button("Clean Special", key=f"clean_spec_{col_name}"):
                    ...
                    st.rerun()
            with action_cols[3]:
                if st.button("Trim", key=f"trim_{col_name}"):
