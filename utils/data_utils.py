...
def generate_column_suggestions(column, series, null_pct, unique_pct, special_chars, outlier_info, fuzzy_keys, blank_count=None):
    ...
    suggestions = []
    if null_pct > 0:
        severity = "Critical" if null_pct > 50 else "Warning" if null_pct > 10 else "OK"
        suggestions.append(f"[{severity}] {null_pct:.1f}% missing values")
    
    if blank_count and blank_count.get('empty_string_count', 0) > 0:
        suggestions.append(f"{blank_count['empty_string_count']} empty strings (not null)")
    
    if blank_count and blank_count.get('whitespace_only_count', 0) > 0:
        suggestions.append(f"{blank_count['whitespace_only_count']} whitespace-only values")
    ...
    if unique_pct < 5:
        suggestions.append("Low cardinality - consider categorical")
    
    if pd.api.types.is_string_dtype(series):
        sample = series.dropna().astype(str)
        if any(s != s.strip() for s in sample):
            suggestions.append("Leading/trailing whitespace")
